...
using Akh.Breed.Web.Areas.AppAreaName.Models.CustomizableDashboard;
...
{
    public abstract class CustomizableDashboardControllerBase : BreedControllerBase
    {
        ...
        public async Task<PartialViewResult> AddWidgetModal(string dashboardName, string pageId)
        {
            var userDashboard = await DashboardCustomizationAppService.GetUserDashboard(
                new GetDashboardInput
                {
                    ...
                    Application = BreedDashboardCustomizationConsts.Applications.Mvc
                }
            );
